...
//    private List<XtToHopMonThi> listMon;
    ...
    public BigDecimal getDiemTT(String maNganh) {
        return nganhDAO.getDiemTTByMaNganh(maNganh);
    }
...
dto.setcongthucDiemDia(diemThiGiaLap.getDi() != null ? 
    ...
    public TinhDiemDGNL tinhDiemDGNL(String nganh, String diemCong, String khuVuc, String doiTuong, String diemThi) {
        ...
        XtBangQuyDoi quyDoi = listQuyDoi.stream().filter(q -> (q.getDDiema().compareTo(diemThiDGNL) <= 0 && q.getDDiemb().compareTo(diemThiDGNL) >= 0 && q.getDPhuongthuc().equals("DGNL") && q.getDTohop().equals(toHopGoc)))
                                                .findAny()
                                                .orElse(null);
